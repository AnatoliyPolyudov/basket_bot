...
case "$1" in
    start|start-auto)
        ...
        SESSION_NAME="basket_bot_${PRESET}"
        
        echo -e "${GREEN}🚀 STARTING BOT IN SCREEN WITH PRESET: $PRESET${NC}"
        
        # Проверяем не запущен ли уже
        if screen -list | grep -q "$SESSION_NAME"; then
            print_warning "Bot already running in screen session: $SESSION_NAME"
            echo "Use: screen -r $SESSION_NAME"
            exit 1
        fi
        
        if pgrep -f "python monitor.py" > /dev/null; then
            print_warning "Bot process already running!"
            echo "Use: ./run_bot.sh stop"
            exit 1
        fi
        
        cd "$BOT_DIR"
        
        # Создаем screen сессию и запускаем
        screen -dmS "$SESSION_NAME" bash -c "
            cd '$BOT_DIR'
            source venv/bin/activate
            echo '🤖 Starting basket bot with preset: $PRESET'
            python monitor.py --preset '$PRESET'
            echo ''
            echo '🔴 Bot stopped. Screen session will close in 30 seconds...'
            sleep 30
        "
        
        if [ $? -eq 0 ]; then
            print_status "Bot started in screen session: $SESSION_NAME"
            echo ""
            echo "📺 SCREEN COMMANDS:"
            echo "   screen -r $SESSION_NAME    - Attach to view bot"
            echo "   screen -ls                 - List all sessions"
            echo "   Ctrl+A, then D             - Detach from screen"
            echo ""
            echo "🔍 Check status: ./run_bot.sh status"
            echo "📋 View logs:    ./run_bot.sh logs"
        else
    ...
    start-direct)
        PRESET="${2:-ultra_liquid_8}"
        echo -e "${GREEN}🚀 STARTING BOT DIRECTLY (NO SCREEN): $PRESET${NC}"
        echo "⚠️  Warning: Bot will stop if SSH connection is lost!"
    ...
    attach)
        ...
        if screen -list | grep -q "$SESSION_NAME"; then
            ...
            screen -ls | grep "basket_bot" || echo "No basket_bot sessions"
        fi
        ;;
    ...
    detach)
        print_info "DETACH FROM SCREEN:"
        echo "When inside screen session, press:"
        echo "   Ctrl+A, then D"
        ;;
...
"
    ...
    help|--help|-h|*)
        ...
        echo "  $0 start [preset]       - Start in SCREEN (recommended)"
        echo "  $0 start-auto           - Start ultra_liquid_8 in screen"
        echo "  $0 start-direct [preset] - Start directly (no screen)"
        echo "  $0 start-default        - Start default in screen"
        echo ""
        ...
        echo "  $0 screens              - List screen sessions"
