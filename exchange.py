...
OKXExchange:
    ...
    def load_markets(self) -> None: try: self.markets = 
    ...
    def get_swap_symbols(self) -> List[str]: symbols = 
        [] for symbol, market in self.markets.items():
            if (market['swap'] and market['active'] and 
    ...
    -> Dict:
        try: order_params = { 'symbol': symbol, 'type': 
            ...
            if price: order_params['price'] = price
    ...
    def close_position(self, symbol: str) -> bool: try: 
            ...
            for position in positions: side = 'sell' if 
                position['side'] == 'long' else 'buy' 
                self.create_order(
